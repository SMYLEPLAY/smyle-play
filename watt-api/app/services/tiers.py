...
def normalize_tier(value: object) -> UserTier:
    ...
    NB : court-circuit si `value` est DÉJÀ un UserTier — sinon le piège
    `str(UserTier.PREMIUM) == "UserTier.PREMIUM"` (et non "premium", en Python
    3.10/3.11 pour un (str, Enum)) ferait retomber tout membre d'enum sur
    STANDARD. En prod les valeurs viennent de la DB en chaînes, mais ce
    court-circuit garantit la justesse quel que soit le type d'entrée.
    """
