...
Garanties posées ici :
  ...
  - UPDATE → seules `status` et `completed_at` peuvent changer (cycle de vie
    PENDING→COMPLETED/FAILED/ROLLED_BACK). Tout changement d'un champ financier
    (type, buyer/seller, montants, split, euro, métadonnées, clé d'idempotence,
    created_at, id) → INTERDIT.
...
BEGIN
  ...
  END IF;
  -- UPDATE : seuls status et completed_at peuvent changer.
