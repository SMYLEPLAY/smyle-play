...
async def accept_adn_offer_atomic(db, *, offer) -> _AcceptAdnOfferResult:
    ...
    async with db.begin_nested():
        ...
        # Débit acheteur (UPDATE additif, même pattern que unlocks.py :
        # smyleplay-api n'a pas de sous-soldes promo/achetés/gagnés).
        await db.execute(
            text(
                "UPDATE users "
        ...
        # Crédit vendeur (part artiste) — calque unlocks.py.
        await db.execute(
            text(
                "UPDATE users "
