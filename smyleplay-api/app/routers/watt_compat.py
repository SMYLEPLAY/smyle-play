...
async def build_artist_detail_payload(
    ...
    # Tracks de l'artiste — on exclut les sons soft-deletés (is_deleted) pour
    # que le profil public respecte les suppressions faites par l'artiste
    # (parité avec /tracks/me, marketplace, récents). Sans ça, un son supprimé
    # restait visible sur /u/<slug>.
    stmt_tracks = (
        select(Track)
        .where(Track.artist_id == target.id, Track.is_deleted.is_(False))
