...
(function initMarketplace() {
  ...
  function _renderGridSons(filter = '') {
    ...
    // TAXONOMIE son/image/œuvre (2026-07-15) — œuvres (avec image vendable
    // liée) d'abord, rendues en CARTES avec cover ; puis les sons seuls,
    // rendus en BARRES musicales sans cover (branche plus bas dans le map).
    items = items.filter(x => x.linkedImage && x.linkedImage.id)
      .concat(items.filter(x => !(x.linkedImage && x.linkedImage.id)));
    el.innerHTML = items.map(t => {
      ...
      // TAXONOMIE — SON SEUL (pas d'image vendable liée) → BARRE musicale sans
      // cover. La cover réapparaît dès que l'artiste ajoute une image vendable
      // (le son repasse alors en carte œuvre). On réutilise .mp-son-card pour
      // hériter de toute la mécanique (play, drawer, recette, like, playlist).
      if (!li) {
        const _recipeBar = (promptId && promptPrice != null)
          ? `<button type="button" class="mp-recipe-badge mp-socle-price mp-son-bar-recipe" data-prompt-id="${_esc(String(promptId))}" data-prompt-price="${promptPrice}" data-track-name="${_esc(title)}" title="Débloquer la recette · ${promptPrice} Smyles">🎧 Recette · ${promptPrice} S</button>`
          : '';
        return (
          `<div class="mp-son-card mp-son-card--bar" data-track-id="${_esc(t.id || '')}" data-stream-url="${_esc(streamUrl)}" data-artist-slug="${_esc(artistSlug)}" style="--son-color:${_esc(color)}">` +
            `<button class="mp-son-card-play" type="button" aria-label="Lire / Pause">` +
              `<svg viewBox="0 0 24 24"><polygon points="5,3 19,12 5,21"/></svg>` +
            `</button>` +
            `<div class="mp-son-bar-main">` +
            ...
            `</div>` +
            (streamUrl ? `<audio preload="none" class="mp-son-card-audio" src="${_esc(streamUrl)}"></audio>` : '') +
          `</div>`
        );
      }
